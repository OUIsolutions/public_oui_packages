...
function PushBlind.actions.install()
    ...
    if  version then
        ...
        os.execute('curl -L https://github.com/OUIsolutions/VibeScript/releases/download/' .. version .. '/linux_bin.out -o darwin.out')

    end 
    if not version then
        os.execute('curl -L https://github.com/OUIsolutions/VibeScript/releases/latest/download/linux_bin.out -o darwin.out') 
    end
